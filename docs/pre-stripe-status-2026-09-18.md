...
**EXTENDED, 2026-09-18 (Cowork, Opus) — see `docs/EMAIL-BAA-DECISION-2026-09-18.md`.** Report 5's Resend quote **stands as written**; an earlier version of this note wrongly said it could not be reproduced, and Claude Code corrected it below — the sentence is in the page's collapsed FAQ/JSON-LD, which a rendered-text fetch misses. Resend also shows a **"HIPAA — In progress"** badge, which is consistent with it: no BAA today, HIPAA on the roadmap. Ask Resend for a dated, written answer. What is genuinely new: the four unresearched vendors (Anthropic, Google Calendar, Twilio, Retell's model provider) are now resolved, along with four email providers that will sign a BAA, two that will not (Postmark and SendGrid both refuse), and the repo's real migration surface (19 client constructions, 27 send sites, no wrapper).
