...
- ✅ Customer's own name was never actually captured — Chris caught this live, mid-checkout, on a fresh signup attempt. `session.customer_details.name` (read by the webhook as `ownerName`) only populates when `billing_address_collection` is `"required"`; all 3 Payment Links had it at Stripe's default `"auto"`, which doesn't show a name field. Real impact: `ownerName` feeds Chris's own internal new-customer notification email (`Owner: ${ownerName}`) and the monthly ROI report contact line — both would have shown "Unknown" for every real signup. Set to `"required"` on all 3 live Payment Links, verified via re-fetch. Live Stripe config change, no code touched.

All four original gaps closed, plus three more found along the way (missing dashboard link in the welcome email, preferred area code never actually reachable, customer name never actually captured). Nothing left on this item except the real-signup verifications noted above (billing portal click-through, area code, and now name capture).
